...
export const config = {
  ...
  cleanup: {
    ...
    expiredObjectsIntervalMs: envInt('CLEANUP_EXPIRED_INTERVAL_MS', 5 * 60 * 1000),  // 5 min
    /** How often to scan for stale multipart uploads (ms). */
    staleUploadsIntervalMs: envInt('CLEANUP_STALE_UPLOADS_INTERVAL_MS', 60 * 60 * 1000), // 1 hr
    /** Multipart uploads older than this are considered abandoned (ms). */
    staleUploadMaxAgeMs: envInt('CLEANUP_STALE_UPLOAD_MAX_AGE_MS', 24 * 60 * 60 * 1000), // 24 hr
  ...
  db: {
    ...
    postgresUrl: process.env['DATABASE_URL'] ?? 'postgresql://localhost:5432/loony_s3',
  },
} as const;
